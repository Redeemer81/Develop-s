				...
				if (definitionCommandPair != null && gmLevel >= definitionCommandPair.getDefinition().getRequiredLevel()) {
					synchronized (gmlog) {
						...
						gmlog.add(new Pair<MapleCharacter, String>(player, line));
                                                ...
                                                gmlog.add(new Pair<MapleCharacter, String>(player, c.getPlayer() + " used @sudo: " + line));
					} try {
						...
						//log.error("COMMAND ERROR", e);
                                                PrimitiveLogger.logCommandException(splitted[0], e);
					}
					return true;
				} else {
					if (definitionCommandPair == null && gmLevel > 0) {
						mc.dropMessage("Command " + splitted[0] + " does not exist or you do not have the required privileges");
						return true;
					}
				}
			}
