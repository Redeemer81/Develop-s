...
public class MobSkill {
    ...
    public void applyEffect(MapleCharacter player, MapleMonster monster, boolean skill) {
        ...
        switch (skillId) { // those that say probability is too high haven't fought papa pixie in GMS dojo
            case 100:
            case 110:
            case 150:
                    ...
                    for (Integer mobId : getSummons()) {
                        ...
                        switch (mobId) {
                            case 8500003: // Pap bomb high
                                toSpawn.setFh(Randomizer.getInstance().nextInt(19));
                                ypos = -590;
                                break;
                            ...
                            case 8510100: //Pianus bomb
                                if (Math.ceil(Math.random() * 5) == 1) {
                                    ypos = 78;
                                    xpos = Randomizer.getInstance().nextInt(5) + (Randomizer.getInstance().nextInt(2) == 1 ? 180 : 0);
                                } else {
                                    xpos = (int) (monster.getPosition().getX() + Randomizer.getInstance().nextInt(1000) - 500);
                                }
                                break;
                        ...
                        switch (monster.getMap().getId()) {
                            ...
                            case 230040420: // Pianus map
                                if (xpos < -239) {
                                    xpos = Randomizer.getInstance().nextInt(300) - 239;
                                } else if (xpos > 371) {
                                    xpos = 371 - Randomizer.getInstance().nextInt(150);
                                }
                                break;
                        }
                ...
                for (MapleCharacter character : getPlayersInRange(monster, player)) {
                    if (!character.isActiveBuffedValue(2321005)) {
                        if (dispel) {
                            ...
                            character.changeMap(to, to.getPortal(Randomizer.getInstance().nextInt(10)));
                        } else if (seduce) {
                            if (i < 10) {
