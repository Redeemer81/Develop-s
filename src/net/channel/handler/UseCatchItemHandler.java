...
public final class UseCatchItemHandler extends AbstractMaplePacketHandler {
        ...
        switch (type) {
            ...
            case 0x08: // osiris put this here
                if (c.getPlayer().getMap().getMonsterByOid(monsobid).getId() == 9300101) {
                    doCapture(c, monsobid, itemid, 0, 1902000);
                }
                break;
            case 0x0D:
                ...
                if (c.getPlayer().getMap().getMonsterByOid(monsobid).getId() == 9300157 && monsHp < (monsMaxHp / 2)) {
                    if (monsMaxHp / 3 + monsHp <= Randomizer.getInstance().nextInt(monsMaxHp)) {
                        doCapture(c, monsobid, itemid, 1, 4031868);
    ...
    private void doCapture(MapleClient c, int monsobid, int itemid, int type, int itemgain) {
        c.getSession().write(MaplePacketCreator.catchMonster(monsobid, itemid, (byte) type));
        ...
        MapleInventoryManipulator.addById(c, itemgain, (short) 1);
    }
}
