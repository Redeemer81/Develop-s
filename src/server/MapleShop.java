...
public class MapleShop {
        ...
        if (item != null && item.getPrice() > 0) {
            if (c.getPlayer().getMeso() >= item.getPrice() * quantity) {
                if (item.getItemId() != itemId || quantity <= 0) {
                    c.getPlayer().ban("STORE HACK", true);
                    return;
                }
                ...
                c.getSession().write(MaplePacketCreator.confirmShopTransaction(c, (byte) 0));
    ...
    public void sell(MapleClient c, MapleInventoryType type, byte slot, short quantity) {
        ...
        if (quantity <= iQuant && iQuant > 0) {
            ...
            c.getSession().write(MaplePacketCreator.confirmShopTransaction(c, (byte) 0x4, getNpcId(), items));
        }
    }
    ...
    public void recharge(MapleClient c, byte slot) {
        ...
        if (item.getQuantity() < slotMax) {
            ...
            if (c.getPlayer().getMeso() >= price) {
                ...
                c.getSession().write(MaplePacketCreator.confirmShopTransaction(c, (byte) 0x8));
            } else {
