...
public class MapleMapFactory {
    ...
    public MapleMap getMap(int mapid, boolean respawns, boolean npcs) {
            return getMap(mapid, respawns, npcs, true, true, false);
    }


    public MapleMap getMap(int mapid, boolean respawns, boolean npcs, boolean reactors, boolean monsters, boolean forceReload) {
        ...
        if ((map == null) || (forceReload)) {
            synchronized (this) {
                ...
                map.setDefaultBGM(MapleDataTool.getString(mapData.getChildByPath("info/bgm"), ""));
                ...
                if (mapid < 140000000) {
                    if (mapid == 130000100 || mapid == 100000201 || mapid == 101000003 || mapid == 102000003 || mapid == 103000003 || mapid == 120000101) {
                        try { // level 120, 200 maps only
                ...
                for (MapleData life : mapData.getChildByPath("life")) {
                 ...
                 //   if (myLife == null)
                   //     continue;
                    if ((myLife instanceof MapleMonster) && monsters) {
                        MapleMonster monster = (MapleMonster) myLife;
                        if(monster.getBanish() != null)
                            continue;
                        ...
                        if ((mobTime == -1) && respawns) {//does not respawn, force spawn once
                            ...
                            map.addMonsterSpawn(ID, mobTime, position, mobile);
                        }
                    } else if (npcs) {
                        map.addMapObject(myLife);
                    }
                }
                if ((mapData.getChildByPath("reactor") != null) && reactors) {
