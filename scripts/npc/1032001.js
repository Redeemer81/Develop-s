...
function start() {
    if (cm.getJobId() == 0) {
        actionx["1stJob"] = true;
        if (cm.getLevel() >= 10)
            cm.sendNext("Want to be a magician? There are some standards to meet. because we can't just accept EVERYONE in... #bYour level should be at least 10, with your INT over 25#k. Let's see.");
        else {
    ...
    } else if (actionx["3thJobI"] || (cm.getPlayer().gotPartyQuestItem("3") && cm.getLevel() >= 70 && cm.getJobId() % 10 == 0 && parseInt(cm.getJobId() / 100) == 2 && !cm.getPlayer().gotPartyQuestItem("P"))){
    ...
    } else if (cm.getPlayer().gotPartyQuestItem("P") && !cm.haveItem(4031059)){
        cm.sendNext("Please, bring me the #b#t4031059##k.");
        cm.dispose();
    } else if (cm.haveItem(4031059) && cm.getPlayer().gotPartyQuestItem("P")){
...
function action(mode, type, selection) {
    ...
    if (actionx["1stJob"]){
        ...
        if (status == 0){
            ...
            cm.sendNext("Alright, you're the " + (job == 210 ? "#bWizard (Fire / Poison)#k" : job == 220 ? "#bWizard (Ice / Lighting)#k" : "#bCleric#k") + " from here on out. Hunters are the intelligent bunch with incredible vision, able to pierce the arrow through the heart of the monsters with ease... please train yourself each and everyday. We'll help you become even stronger than you already are.");
            if (cm.getJobId() != job)
                cm.changeJobById(job);
        } else if (status == 4)
            cm.sendNextPrev("I have just given you a book that gives you the list of skills you can acquire as a " + (job == 210 ? "wizard (Fire / Poison)" : job == 220 ? "wizard (Ice / Lighting)" : "cleric") + ". Also your etc inventory has expanded by adding another row to it. Your max HP and MP have increased, too. Go check and see for it yourself.");
        else if (status == 5)
        ...
        if (status == 0){
            if (cm.getPlayer().gotPartyQuestItem("3")){
                cm.getPlayer().removePartyQuestItem("3");
                cm.getPlayer().setPartyQuestItemObtained("P");
            }
        ...
        cm.getPlayer().removePartyQuestItem("P");
