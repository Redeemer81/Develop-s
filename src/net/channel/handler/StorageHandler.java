...
public final class StorageHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == 4) { // take out
            ...
            if(playerMesos < 0) //infinite meso glitch patch
                return;
            ...
            if ((meso > 0 && storageMesos >= meso) || (meso < 0 && playerMesos >= -meso)) {
                ...
                int overflow = c.getPlayer().updateMesosGetOverflow(meso);
                if (overflow > 0 && ((storage.getMeso() + overflow) <= Integer.MAX_VALUE))
                    storage.setMeso(storage.getMeso() + overflow);
