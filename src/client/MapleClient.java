...
public class MapleClient {
    ...
    public boolean hasBannedIP() {
        boolean ret = false;
        try {
            PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT COUNT(*) FROM ipbans WHERE ip RLIKE ?");
    ...
    public int login(String login, String pwd, boolean ipMacBanned) {
        return login(login, pwd, ipMacBanned, false);
    }

    public int login(String login, String pwd, boolean ipMacBanned, boolean justCheck) {
        loginattempt++;
        if (loginattempt > 6) {
        ...
        Connection con = DatabaseConnection.getConnection();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT id, password, salt, banned, gm, socialNumber, pin, beta, greason, tempban FROM accounts WHERE name = ?");
            ...
            if (rs.next()) {
                ...
                socialNumber = rs.getInt("socialNumber");
                pin = rs.getString("pin");
                Beta = rs.getInt("beta") == 1;
                ...
                if (banned > 0) {
                    ...
                    if (getLoginState() > LOGIN_NOTLOGGEDIN && !justCheck) { // already loggedin
                        loggedIn = false;
                        loginok = 7;
...
//            player.getFamily().setPlayer(null);
    ...
    public ScriptDebug getScriptDebug()
    {
        return this.scriptDebug;
    }

    public boolean isWorldSent()
    {
        return worldSent;
    }

    public void setWorldSent(boolean state)
    {
        worldSent = state;
    }
