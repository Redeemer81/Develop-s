...
public final class PartyOperationHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        switch (operation) {
            ...
            case 3: { // accept invitation (lol used to be 3)
            ...
            case 4: { // invite
                ...
                if (invited != null) {
                    if (invited.getParty() == null) {
                        if (party.getMembers().size() < 6) {
                            c.getSession().write(MaplePacketCreator.partyStatusMessage(26, name));
                            invited.getClient().getSession().write(MaplePacketCreator.partyInvite(player));
                        }
                    } else {
                        c.getSession().write(MaplePacketCreator.partyMessage("'" + name + "' 님은 이미 가입한 파티가 있습니다."));
                    }
                } else {
                    c.getSession().write(MaplePacketCreator.partyMessage("현재 서버에서 '" + name + "' 님을 찾을 수 없습니다."));
                }
                break;
            }
            case 5: { // expel
