...
function start() {
    ...
    if (cm.getPlayer().getPosition().x > -148 && cm.getPlayer().getPosition().x < 421 && cm.getPlayer().getPosition().y < 554) {
        if (cm.isQuestStarted(2057) && cm.canHold(4031041)){
            cm.sendNext("Looking carefully into Treasure Chest, there seems to be a bag."); //Not GMS like
            return;
        } else if (!cm.canHold(4031041))
        ...
        cm.sendNext("You need to be closer.");
    cm.dispose();
}
