...
public abstract class AbstractDealDamageHandler extends AbstractMaplePacketHandler {
    ...
    protected void applyAttack(AttackInfo attack, MapleCharacter player, int attackCount) {
        ...
        if(!ignoreDMGCheck(attack, player) && attackCount != attack.numDamage) {
            return;
        }
     ...
     /*           if (player.getBuffedValue(MapleBuffStat.COMBO_DRAIN) != null) {
                ...
                if (player.getBuffedValue(MapleBuffStat.COMBO_DRAIN) != null && !(player.getHp() == player.getMaxHp())) {
                ...
                if(totDamageToOneMonster >= 999999)
                ...
                if (attack.isHH && !monster.isBoss()) {
                 ...
                 if (player.getBuffedValue(MapleBuffStat.DARKSIGHT) != null && player.getJob().isA(MapleJob.BLADE_LORD)) {
                    final ISkill skill = SkillFactory.getSkill(BladeLord.ADVANCED_DARK_SIGHT);
                    final int skillLevel = player.getSkillLevel(BladeLord.ADVANCED_DARK_SIGHT);
    ...
    protected AttackInfo parseDamage(LittleEndianAccessor lea, boolean ranged) {
        return parseDamage(lea, ranged, false);
    }

    protected AttackInfo parseDamage(LittleEndianAccessor lea, boolean ranged, boolean magic) {
        ...
        if (ret.skill == FPArchMage.BIG_BANG || ret.skill == ILArchMage.BIG_BANG || ret.skill == Bishop.BIG_BANG || ret.skill == Gunslinger.GRENADE || ret.skill == Brawler.CORKSCREW_BLOW || ret.skill == ThunderBreaker.CORKSCREW_BLOW || ret.skill == Evan.ICE_BREATH || ret.skill == Evan.FIRE_BREATH || ret.skill == BladeMaster.FINAL_CUT) {
        ...
        if (ranged)
            ...
            lea.skip(6);
        ret.UNK80 = lea.readByte(); // display
        ret.stance = lea.readByte(); // animation
        lea.skip(4);
        ...
        if (ranged)
            lea.skip(4); // ?
        ret.pos = new Point(lea.readShort(), lea.readShort());
        return ret;
    }
    ...
    private static boolean ignoreDMGCheck(AttackInfo attack, MapleCharacter chr)
    {
        ...
        if(chr.getBuffedValue(MapleBuffStat.COMBO_DRAIN) != null)
            return true;
        if (chr.getJob().getId() / 10 == 43 && attack.skill == 0)
            return true;
