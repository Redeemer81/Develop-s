...
public final class PetFoodHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        slea.skip(4);
        slea.readShort(); // itemslot
        if (pet == null) {
            return;
        }
        int itemId = slea.readInt();
        if (itemId / 1000000 == 2 && itemId != 2120000) {
            return;
        }
        ...
        if (pet.getFullness() < 100) {
            ...
            if (gainCloseness && pet.getCloseness() > 0) {
                int newCloseness = pet.getCloseness() - 1;
                pet.setCloseness(Math.max(newCloseness, 0));
                if (newCloseness < ExpTable.getClosenessNeededForLevel(pet.getLevel()) && pet.getLevel() > 1) {
                    pet.setLevel(pet.getLevel() - 1);
                }
            }
            c.getSession().write(MaplePacketCreator.updatePet(pet));
