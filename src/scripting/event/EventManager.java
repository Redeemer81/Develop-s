...
public class EventManager {
    ...
    //loner character ahaha - mainly for testing
    public void startInstance(MapleCharacter ch, MapleMap map) {
        try {
            EventInstanceManager eim = (EventInstanceManager) (iv.invokeFunction("setup", (Object) null));
            eim.registerPlayer(ch);
        } catch (ScriptException ex) {
            Logger.getLogger(EventManager.class.getName()).log(Level.SEVERE, null, ex);
        } catch (NoSuchMethodException ex) {
            Logger.getLogger(EventManager.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void DoubleEXPRate(boolean start)
    {
        try {
        cserv.getWorldInterface().changeRates(start ? -2 : -1, (double)-1, (double)-1, (double)-1);
        } catch (Exception e){
