...
-- By ---------------------------------------------------------------------------------------------
	Xterminator
-- Version Info -----------------------------------------------------------------------------------
        2.0 - Second Version by BubblesDev v75
...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        var text = "The regular fee applies for all non-beginners.";
        if (cm.getJobId() == 0){
            cost *= 10;
            text = "We have a special 90% discount for beginners.";
        }
        cm.sendYesNo(text + " The Ant Tunnel is located deep inside in the dungeon that's placed at the center of the Victoria Island, where the 24 Hr Mobile Store is. Would you like to go there for #b" + cost + " mesos#k?");
    } else if (status == 2) {
        if (cm.getMeso() < cost)
