...
public final class QuestActionHandler extends AbstractMaplePacketHandler {
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (action == 1) {
                ...
                try
                {
                    allowed = c.getChannelServer().getWorldRegistry().getProperty("quests").equals(Boolean.TRUE);
                } catch (RemoteException re) {
                    c.getChannelServer().reconnectWorld();
                }
           // return;//Start Quest
            //temporarily disabled until map starvation issues sorted
            if(!allowed)
            {
              //  player.dropMessage("Quests are temporarily disabled until various issues are fixed. We're sorry for any inconvenience.");
                return;
            } else {
