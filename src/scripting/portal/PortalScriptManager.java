...
public class PortalScriptManager {
    ...
    private PortalScript getPortalScript(String scriptName) {
        ...
        try {
            ...
            ute.printStackTrace();
            System.out.println("Error: Portal " + scriptName + ". UndeclaredThrowableException.");
        } catch (Exception e) {
            System.out.println("Error: Portal " + scriptName + ".");
        }finally {
            if (fr != null) {
                try {
                    fr.close();
    ...
    public boolean executePortalScript(MaplePortal portal, MapleClient c) {
        ...
        if (script != null) {
            ...
            if (c.getPlayer().isGM())
                c.getPlayer().dropMessage("존재하지않는포탈 : " + portal.getScriptName());
        }
        return false;
    }

    	public void clearScripts() { //LALA I WROTE SHOOTSOURCE AND I'M A RETARD SO I TOOK THIS OUT BECAUSE I DONT WANT EASY RECACHING OF SCRIPTS DERRPPP
		scripts.clear();
	}
}