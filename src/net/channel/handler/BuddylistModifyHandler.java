...
public class BuddylistModifyHandler extends AbstractMaplePacketHandler {
    ...
    private void nextPendingRequest(MapleClient c) {
        ...
        if (pendingBuddyRequest != null) {
            c.getSession().write(MaplePacketCreator.requestBuddylistAdd(pendingBuddyRequest.getId(), c.getPlayer().getId(), pendingBuddyRequest.getName(), 0, 0));//todo: real offline levels / jobids
        }
    }
        ...
        if (mode == 1) { // add
                ...
                try {
                    ...
                    if (charWithId != null) {
                        ...
                        if (channel != -1) {
                            ...
                            buddyAddResult = channelInterface.requestBuddyAdd(addName, c.getChannel(), player.getId(), player.getName(), player.getLevel(), player.getJob().getId());
                        } else {
                            Connection con = DatabaseConnection.getConnection();
            ...
            if (!buddylist.isFull()) {
                try {
                    ...
                    if (otherName != null) {
                        buddylist.put(new BuddylistEntry(otherName, "그룹 미지정", otherCid, channel, true));
                        c.getSession().write(MaplePacketCreator.updateBuddylist(buddylist.getBuddies()));
                        notifyRemoteChannel(c, channel, otherCid, ADDED);
                    }
                } catch (RemoteException e) {
