...
function start() {
    if (cm.haveItem(4031012) || (cm.getJobId() >= 110 && cm.getJobId() <= 132)) {
        cm.sendOk("You're truly a hero!");
        cm.dispose();
    } else if (cm.haveItem(4031008))
        cm.sendNext("Hmmm... it is definitely the letter from #b#p1022000##k, so you came all the way here to take the test and make the 2nd job advancement as the warrior? Alright, I'll explain the test to you. Don't sweat it too much, though; it's not that complicated.");
    else {
...
function action(mode, type, selection) {
    ...
    if (status == 0)
	    ...
	    cm.dispose();
	    for (var i = 108000300; i < 108000302; i++){
