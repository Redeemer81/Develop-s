...
function start() {
    if (cm.haveItem(4031012) || (cm.getJobId() >= 410 && cm.getJobId() <= 422)) {
        cm.sendOk("You're truly a hero!");
        cm.dispose();
    } else if (cm.haveItem(4031011))
        cm.sendNext("Hmmm... it is definitely the letter from #b#p1052001##k, so you came all the way here to take the test and make the 2nd job advancement as the thief? Alright, I'll explain the test to you. Don't sweat it too much, though; it's not that complicated.");
    else {
...
function action(mode, type, selection) {
    ...
    if (status == 0)
	    ...
	    cm.dispose();
	    for (var i = 108000400; i < 108000402; i++){
