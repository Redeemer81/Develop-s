...
function action(mode, type, selection){
	...
	if (actionx["Mental"]){
	    if (status == 0)
		    ...
		    if (cm.getJobId() % 10 == 0) {
				...
				cm.getPlayer().removePartyQuestItem("Q");
			}
			//Need thieves text.
	    ...
	    if (status == 0)
		    ...
		    if (cm.haveItem(4031057)){
				...
				cm.getPlayer().setPartyQuestItemObtained("Q");
			}
	...
	} else if (cm.getPlayer().gotPartyQuestItem("3") && selection == 0){
	    cm.sendNext("Go, talk with #b#p1052001##k and bring me #b#t4031057##k.");
		cm.dispose();
	} else if (cm.getPlayer().gotPartyQuestItem("Q") && selection == 0){
