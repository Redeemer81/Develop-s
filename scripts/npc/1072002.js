...
function start() {
    if (cm.haveItem(4031012) ||(cm.getJobId() >= 310 && cm.getJobId() <= 322)) {
        cm.sendOk("You're truly a hero!");
        cm.dispose();
    } else if (cm.haveItem(4031010))
        cm.sendNext("Hmmm... it is definitely the letter from #b#p1012100##k, so you came all the way here to take the test and make the 2nd job advancement as the bowman? Alright, I'll explain the test to you. Don't sweat it too much, though; it's not that complicated.");
    else {
...
function action(mode, type, selection) {
    ...
    if (status == 0)
	    ...
	    cm.dispose();
	    for (var i = 108000100; i < 108000102; i++){
